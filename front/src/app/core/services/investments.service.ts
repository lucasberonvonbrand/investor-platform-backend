  ...
  getInvestmentById(investmentId: number): Observable<IInvestedProject | null> {
    ...
    return this.http.get<IInvestment>(`/api/investments/${investmentId}`).pipe(
      switchMap((investment: IInvestment) => {
        ...
        const project$ = this.projectsMasterSvc.getProjectById(investment.projectId, true).pipe(
          catchError(() => of(null as any))
        );

        // Si la inversión ya trae los profits, genial.
        if (investment.profit1Year != null) {
          return project$.pipe(map(project => ({ ...investment, project } as IInvestedProject)));
        }

        // Si no, los buscamos en los contratos del proyecto como fallback.
        const contracts$ = this.projectsMasterSvc.getContracts(investment.projectId).pipe(
        ...
        return contracts$.pipe(
          switchMap(contracts => {
            ...
            if (!relatedContract) { // Fallback por si no hay contratos (poco probable)
              return project$.pipe(map(project => ({ ...investment, project, earnings: [] } as IInvestedProject))); }

            const earnings$ = this.projectsMasterSvc.getEarningsByContractId(relatedContract.idContract).pipe(
              catchError(() => of([] as any[]))
            );
