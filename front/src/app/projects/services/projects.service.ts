...
export class ProjectsService {
  ...
  getAll(): Observable<IProject[]> {
    return this.http.get<ProjectApi[]>(this.api).pipe(
      ...
      catchError((err) => {
        console.warn('ProjectsService.getAll endpoint error:', err);
        return of([]);
      })
    );
  }
  ...
  getAllByTag(tag: string): Observable<IProject[]> {
    ...
    return this.http.get<ProjectApi[]>(url).pipe(
      map(list => (list ?? []).map(adapt)),
      catchError((err) => {
        console.warn('ProjectsService.getAllByTag endpoint error:', err);
        return of([]);
      })
    );
  }
