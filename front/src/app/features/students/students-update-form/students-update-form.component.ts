 ...
 if (role === 'ROLE_STUDENT' && userId) {
  ...
  private buildForm(student: Student) {
    this.studentsUpdateForm = this.fb.group({
      ...
      firstName: [student.firstName ?? '', [Validators.required, Validators.maxLength(20), Validators.pattern('^[a-zA-ZÀ-ÿ\\s]*$')]],
      lastName: [student.lastName ?? '', [Validators.required, Validators.maxLength(20), Validators.pattern('^[a-zA-ZÀ-ÿ\\s]*$')]],
      dni: [{ value: student.dni ?? '', disabled: true }, [Validators.required, Validators.maxLength(20)]],
      phone: [student.phone ?? '', [Validators.required, Validators.maxLength(10), Validators.pattern(/^\+?\d{8,15}$/)]],
